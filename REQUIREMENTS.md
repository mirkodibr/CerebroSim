...
* [x] **Theme Token Audit:** Replace all hardcoded colors (e.g., `Colors.white`) with `Theme.of(context).colorScheme` tokens for flawless Light/Dark mode transitions.
* [x] **Vault & Save UX:** Add loading states to the save operation and implement client-side filtering/sorting for the Vault gallery.
...
* [x] **Engine Integration:** Inject the user-defined `NetworkConfig` into the `SimulationEngine` and serialize it within `ExperimentSnapshot` saves.
...
* [x] **Tactile & Discoverability:** Implement `HapticFeedback` on interactions, a first-launch 3D gesture hint, and onboarding state restoration.
* [x] **Premium Styling:** Apply high-fidelity UI constraints (compact chips, consistent gap spacing, subtle card borders, flat app bars).
