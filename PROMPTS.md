...
[x] 27. **Sine Wave Tracking Punishment Logic:** Read `lib/providers/environment_provider.dart`. The current punishment logic for the Sine Wave task penalizes *any* movement (`if (lastAction != 'none') return -0.5;`), which will cause the agent to freeze. 
